...
import { DeckListRow } from "@/components/DeckListRow";
...
export default function TeamStarMapPage() {
  ...
  const isAdmin = hasPermission("deck:manage");
  ...
  const showSwitcher = isAdmin || (teamsData?.teams.length ?? 0) > 1;

  const { data: starMap, isLoading: mapLoading } = useQuery<StarMapData>({
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start">
                    ...
                    {teamsData?.teams.map((team) => (
                      <DropdownMenuItem key={team.id} onSelect={() => setSelectedTeamId(team.id)}>
                        {team.name}
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
        ...
        {noTeams ? (
          <div className="rounded-xl border border-dashed py-16 text-center text-muted-foreground">
            ...
            <p className="text-sm max-w-sm mx-auto">
              No teams are assigned to you yet. Ask an admin to create a team in Settings → Teams.
            </p>
          </div>
        ) : mapLoading || !starMap ? (
          ...
          <>
            ...
            <StarMapTable
              ...
              onOpenDrawer={(userId, categorySlug) =>
