# Development workflow (mirrored copy)

**Canonical source:** `bray-scenarios/docs/DEVELOPMENT.md` (also mirrored in
...
## This repo (bray-flashcards)

Flashcard/quiz trainer on published `@heybray/*` platform packages. Single-package app
(NOT Scenarios' client/server workspaces):
...
**No `@heybray/llm`.** Local auth only (no SSO/2FA).

### Non-obvious conventions

1. **Server has NO build step** — runs raw TypeScript via `tsx`.
2. **ESM everywhere** (`"type": "module"`).
3. **Platform gaps go through `bray-platform`** — PR + changeset + npm publish via CI.
