...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      log.error("Failed to get member history", error instanceof Error ? error : undefined);
      res.status(500).json({ message: "Failed to get member history" });
    }
  },
);
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      log.error("Failed to get member sessions", error instanceof Error ? error : undefined);
      res.status(500).json({ message: "Failed to get member sessions" });
    }
  },
);
