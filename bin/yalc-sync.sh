#!/usr/bin/env bash
# yalc publish @heybray/flashcards-* feature packages. See docs/dev-workflow.md.
set -euo pipefail
...
FEATURE_PKGS=(flashcards-server flashcards-client)
...
usage() {
  cat <<EOF
Usage:
  bin/yalc-sync.sh                       # publish + push all flashcards feature packages
  bin/yalc-sync.sh <pkg> [<pkg>…]        # publish named package(s) only
  bin/yalc-sync.sh --link <path>         # yalc add flashcards-server + flashcards-client
...
cmd_link() {
  ...
  echo "Linking flashcards-server + flashcards-client into ${target}"
  (
    cd "$target"
    yalc add @heybray/flashcards-server @heybray/flashcards-client
    npm install
  )
}
...
cmd_status() {
  ...
  if grep -qE '\.yalc|file:\.yalc' "$target/package.json" 2>/dev/null; then
    ...
    grep -E '@heybray/flashcards-|\.yalc|file:\.yalc' "$target/package.json" || true
  else
    echo "package.json: no yalc-linked flashcards packages"
  fi
